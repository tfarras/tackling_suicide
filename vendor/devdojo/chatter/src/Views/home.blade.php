...
<div id="chatter" class="chatter_home" >
	...
	<div id="chatter_hero" style="background: transparent;">
        <img src="/assets/images/logo_white.png" style=" margin-top: 2%; width: 20%;max-height: 15%">
	</div>
	    ...
	    <div class="row">
	    	...
	    	<div class="col-md-3 left-column">
	    		...
	    		<div class="chatter_sidebar">
					<button style="color: white !important;" class="btn btn-primary" id="new_discussion_btn"><i class="chatter-new"></i> New {{ Config::get('chatter.titles.discussion') }}</button>
					<a style="color:white !important;" href="/{{ Config::get('chatter.routes.home') }}"><i class="chatter-bubble"></i> All {{ Config::get('chatter.titles.discussions') }}</a>
					<ul class="nav nav-pills nav-stacked">
						<?php $categories = DevDojo\Chatter\Models\Models::category()->all(); ?>
						@foreach($categories as $category)
							<li><a style="font-size:18px;color:white !important;" href="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.category') }}/{{ $category->slug }}"><div class="chatter-box" style="background-color:{{ $category->color }}"></div> {{ $category->name }}</a></li>
						@endforeach
					</ul>
				</div>
				<!-- END SIDEBAR -->
	        ...
	        <div class="col-md-9 right-column">
	        	<div class="panel" style="border-radius: 30px; border-top:none;" >
		        	<ul class="discussions" style="border-radius: 30px" >
		        		@foreach($discussions as $discussion)
				        	<li >
				        		<a class="discussion_list" onmouseover="this.style.borderRadius='30px'" style="border-top:none"   href="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.discussion') }}/{{ $discussion->category->slug }}/{{ $discussion->slug }}">
					        		<div class="chatter_avatar">
					        			<img src="/avatars/{{$discussion->user->avatar}}">
					        		</div>
	...
	<div id="new_discussion">
    	...
    	<form id="chatter_form_editor" action="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.discussion') }}" method="POST">
			...
			<div id="new_discussion_footer">
				...
				<a style="color:white !important;" href="/{{ Config::get('chatter.routes.home') }}" class="btn btn-default pull-right" id="cancel_discussion">Cancel</a>
